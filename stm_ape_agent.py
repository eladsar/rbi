...
class ApeAgent(Agent):
    ...
    def multiplay(self):
        ...
        while True:
            ...
            s = torch.cat([env.s for env in mp_env])
            # s_hash = [hash(str(env.s.data)) for env in mp_env]
            # add to fifo
            s_fifo.pop(0)
            s_fifo.append(s)
            s = s.to(self.device)
            ...
            for i in range(n_players):
                ...
                # fetch gate
                if self.stm_enable and a_stm[i] >= 0 and self.n_offset >= 4 * self.random_initialization \
                ...
                if len(rewards[i][-1]) > max(self.n_stm, self.history_length):
                    ...
                    scaled_tde = np.clip(np.sign(tde) * np.minimum(np.abs(tde), np.abs(tde / v_scale)), a_min=0, a_max=None)

                    if scaled_tde > np.random.exponential(1/exp_scale):
                ...
                if env.t:
                    ...
                    exp_scale = np.clip(exp_scale * 0.99 ** np.sign(pool - self.stm_pool), a_max=20, a_min=0.01)

                    print("ape | st: %d\t| sc: %d\t| f: %d\t| e: %7g\t| typ: %2d | trg: %d | t: %d\t| n %d\t| avg_r: %g\t| avg_f: %g\t| exp: %g\t| pool: %g\t|" %
                        (self.frame, env.score, env.k, mp_explore[i], np.sign(explore_threshold[i]), mp_trigger[i],
                         ...
                         self.behavioral_avg_frame, float(exp_scale), pool))
